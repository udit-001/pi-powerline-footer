...
// oc-tps-style token-rate formatters
...
const contextPctSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    // When the token count is unknown (e.g. right after compaction), show an
    // explicit placeholder instead of a stale/zero percentage.
    const pctText = ctx.contextUnknown ? "…" : `${pct.toFixed(1)}%`;
    const text = `${pctText}/${formatTokens(window)}${autoIcon}`;
...
// Token-rate segments (oc-tps style).
// Combined: shows live TPS · avg TPS · avg TTFT (whenever a sample exists).
const tokenRateSegment: StatusLineSegment = {
  id: "token_rate",
  render(ctx) {
    const icons = getIcons();
    const tr = ctx.tokenRate;
    const parts: string[] = [];

    if (tr.isStreaming || tr.generationCount > 0) {
      parts.push(formatRate(tr.liveTps));
      if (tr.avgTps > 0) parts.push(`avg ${formatRate(tr.avgTps)}`);
      if (tr.avgTtftMs > 0) parts.push(`ttft ${formatTtft(tr.avgTtftMs)}`);
    }

    if (parts.length === 0) return { content: "", visible: false };
    const content = withIcon(icons.tokens, parts.join(` ${SEP_DOT} `));
    // Accent while streaming, calm when idle after a generation.
    const semantic: SemanticColor = tr.isStreaming ? "tokens" : "context";
